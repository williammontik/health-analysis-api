<!-- === START HEALTH WIDGET (English, lang=en) === -->
...
<style>
  ...
  #resultContainerHealth { opacity: 0; transition: opacity .5s; }
  #resultContainerHealth.show { opacity: 1; }

  #resultContainerHealth, #resultContainerHealth * {
...
<div id="hiddenFormHealth">
  <div style="background:#f9f9f9;padding:20px;border-left:6px solid #4CAF50;border-radius:8px;margin:20px 0;">
    <p style="font-size:18px;font-weight:bold;color:#4CAF50;">
      🌱 Let’s explore how your health data can unlock new insights.
    </p>
    ...
    <p style="margin-top:10px;font-size:15px;line-height:1.7;color:#333;">
      Your data is kept confidential and used solely for personalized analysis. 🛡
    </p>
  </div>
  ...
  <form id="healthForm" style="display:flex;flex-direction:column;gap:15px;pointer-events:none;opacity:0.3;">
    ...
    <select id="country" required disabled>
      <option value="">Please Select</option>
      <option>Singapore</option><option>Malaysia</option><option>Taiwan</option>
    </select>

    <label>📌 Main Concern</label>
    <select id="condition" required disabled>
      <option value="">Please select</option>
    ...
    <textarea id="details" maxlength="300" placeholder="Describe your health habits, lifestyle, or anything relevant..." required disabled></textarea>

    <label>🙌 Referrer (if any)</label>
    <input type="text" id="referrer" placeholder="e.g. Mr. Lee, h/p: 98765432" disabled>

    <label>🪽 Wellness Pal (optional)</label>
    <input type="text" id="angel" placeholder="Enter code if you received one" disabled>

    <button type="submit" id="submitButtonHealth" style="padding:14px;background:#4CAF50;color:#fff;border:none;border-radius:10px;cursor:pointer;" disabled>
      🚀 Submit
    </button>
  </form>
</div>
...
<div id="resultContainerHealth" style="display:none;margin-top:20px;"></div>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async e => {
      ...
      const dob = `${document.getElementById('dob_year').value}-${String(document.getElementById('dob_month').value).padStart(2, '0')}-${String(document.getElementById('dob_day').value).padStart(2, '0')}`;
