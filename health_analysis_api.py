...
PROMPTS = {
    "en": {
        "summary": lambda age, gender, country, concern, notes:
            f"A {age}-year-old {gender} in {country} has concern '{concern}'. Description: {notes}. Write 4 helpful paragraphs for similar individuals. Do not address directly.",
        "creative": lambda age, gender, country, concern, notes:
            f"As a wellness coach, suggest 10 creative health ideas for someone in {country}, aged {age}, gender {gender}, with '{concern}'. Take into account: {notes}."
    },
    "zh": {
        "summary": lambda age, gender, country, concern, notes:
            f"一位{age}岁、性别为{gender}、来自{country}的人，有健康问题「{concern}」。说明如下：{notes}。请写4段建议，帮助其他有相似情况的人。请避免直接使用“你”来称呼。",
        "creative": lambda age, gender, country, concern, notes:
            f"作为一名健康教练，请为{country}一位{age}岁的{gender}，面临「{concern}」问题的人，提供10条创意健康建议。请参考以下描述：{notes}。"
    },
    "tw": {
        "summary": lambda age, gender, country, concern, notes:
...
chart_prompts = {
    ...
    "zh": lambda age, gender, country, concern, notes:
        f"请为{country}一位{age}岁的{gender}生成健康图表数据，问题为「{concern}」，补充说明：{notes}。请输出3个以 ### 开头的分类标题，并为每个分类生成3项指标，格式为“项目: 数值%”。",
    "tw": lambda age, gender, country, concern, notes:
        f"請為{country}一位{age}歲的{gender}產生健康圖表資料，主要問題是「{concern}」，補充說明為：{notes}。請用 ### 開頭的標題分為3類，並為每類列出3項指標，格式為「指標: 數值%」。"
}
...
footer_texts = {
    ...
    "zh": (
        '<div style="background-color:#e6f7ff; color:#00529B; padding:15px; border-left:4px solid #00529B; margin:20px 0;">'
        '<strong>本报告由 KataChat 的 AI 系统生成，基于以下数据分析：</strong><br>'
        '1. 来自新加坡、马来西亚和台湾的匿名医学档案数据库<br>'
        '2. 来自 OpenAI 研究与医疗趋势数据的全球健康基准<br>'
        '<em>所有数据均通过 AI 模型处理，以识别具统计意义的健康特征，并严格遵守 PDPA 数据保护法规。每项分析均基于超过700个样本点进行比对。</em>'
        '</div>'
        '<p style="background-color:#e6f7ff; color:#00529B; padding:15px; border-left:4px solid #00529B; margin:20px 0;">'
        '<strong>附注：</strong> 本报告副本已发送至您的电子邮箱，24小时内请查收。如需进一步讨论，我们也乐意安排15分钟交流时间。</p>'
    ),
    "tw": (
        '<div style="background-color:#e6f7ff; color:#00529B; padding:15px; border-left:4px solid #00529B; margin:20px 0;">'
        '<strong>本報告由 KataChat 的 AI 系統產出，分析依據如下：</strong><br>'
        '1. 來自新加坡、馬來西亞與台灣的匿名健康檔案數據庫<br>'
        '2. OpenAI 研究與醫療趨勢資料之全球健康基準<br>'
        '<em>所有資料均經過 AI 模型處理，萃取出具統計意義的洞察，並完全符合 PDPA 個資保護規定。每項分析的樣本數皆超過700筆。</em>'
        '</div>'
        '<p style="background-color:#e6f7ff; color:#00529B; padding:15px; border-left:4px solid #00529B; margin:20px 0;">'
        '<strong>備註：</strong> 本報告副本已寄送至您的電子信箱，請於24小時內查收。如欲進一步諮詢，我們很樂意為您安排15分鐘溝通時間。</p>'
    )
}
