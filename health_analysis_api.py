...
footer_texts = {
    ...
    "zh": {
        "main": (
            "<strong>本报告内容由 KataChat 的 AI 系统生成，基于以下数据分析：</strong><br>"
            "1. 新加坡、马来西亚和台湾匿名健康档案的专有数据库<br>"
            "2. 来自 OpenAI 研究与全球健康趋势报告的汇总指标<br>"
            "<em>所有数据经过我们 AI 模型处理，确保统计结果具代表性，并严格遵守 PDPA 隐私条例。"
            "每项分析的样本量至少为 700 个健康记录。</em>"
        ),
        "ps": (
            "<strong>附注：</strong> 这份报告也已通过电子邮件发送至您的邮箱，预计 24 小时内送达。"
            "如需进一步探讨，我们很乐意安排 15 分钟的咨询通话。"
        )
    },
    "tw": {
        "main": (
            "<strong>本報告由 KataChat 的 AI 系統產出，分析來源如下：</strong><br>"
            "1. 來自新加坡、馬來西亞與台灣的匿名健康資料庫<br>"
            "2. 綜合 OpenAI 研究與國際健康趨勢報告的全球指標<br>"
            "<em>所有數據皆經 AI 模型計算與篩選，並嚴格遵守 PDPA 隱私條例。"
            "分析樣本量皆超過 700 筆以上，確保具統計意義。</em>"
        ),
        "ps": (
            "<strong>備註：</strong> 本報告副本也已寄送至您的電子信箱，請於 24 小時內查收。"
            "如需進一步討論，我們很樂意協助安排 15 分鐘的通話時間。"
        )
    }
}
...
def health_analyze():
    try:
        ...
        footer = (
            f"<div style='background-color:#e6f7ff; color:#00529B; padding:15px; border-left:4px solid #00529B; margin:20px 0;'>"
            f"{footer_texts.get(lang, footer_texts['en'])['main']}</div>"
            f"<p style='background-color:#e6f7ff; color:#00529B; padding:15px; border-left:4px solid #00529B; margin:20px 0;'>"
            f"{footer_texts.get(lang, footer_texts['en'])['ps']}</p>"
        )
